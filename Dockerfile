...
# and `pip install breast-cancer-toolkit` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "breast-cancer-toolkit", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n breast-cancer-toolkit -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "breast-cancer-toolkit", "/bin/bash", "-c"]
...
ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "breast-cancer-toolkit", "breast-cancer-toolkit web"]
